...
package org.bytearray.gif.encoder
{
	...
	public class GIFEncoder
	{
		...
		private var useGCP:Boolean=false; // Default to always generate a LCP for each frame
		...
		*/
		...
		/**
		 * Sets whether each frame will use the global color palette (generated by the
		 * first frame added) or generate its own local color palette
		 */
		public function useGlobalColorPalette(enabled:Boolean):void
		{
			useGCP=enabled;
		}
		...
		*/
		...
		public function addFrame(im:BitmapData):Boolean
		{
		    ...
		    try {
		      ...
		      if (!firstFrame && !useGCP) writePalette(); // local color table
		...
		*/
		...
		private function analyzePixels():void
		{
			...
			// reuse processed quantizer if using GCP
			if(!useGCP || firstFrame)
			{
		...
		*/
		...
		private function writeImageDesc():void
		{
		    ...
		    // packed fields
		    if (firstFrame || useGCP) {
		      // no LCT - GCT is used for first (or only) frame or all frames if using GCP
		      out.writeByte(0);
		    } else {
		      // specify normal LCT
